...
func getArgsFromLambdaEvent(ctx context.Context, event json.RawMessage, cmcAPIKey string) ([]string, error) {
	...
	case "validate":
		args = append(args, "--market-map", "generated-market-map.json", "--cmc-api-key", cmcAPIKey, "--oracle-config", "./local/fixtures/e2e/oracle.json", "--start-delay", "10s", "--duration", "1m", "--enable-all")
	case "upserts":
